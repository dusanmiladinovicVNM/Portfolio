...
Only browser-safe values belong in `VITE_*`. In Vercel, configure public-framework-prefixed `VITE_*` values with **Config** visibility; Vercel does not allow them to use Secret visibility because their values are embedded in the browser bundle. Never put the Supabase service-role key, database password, Google client secret, Google refresh token, or any other server credential into Vercel frontend variables.
...
Supabase Edge Function secret updates are available to hosted functions immediately, so changing `PORTFOLIO_WEB_ORIGIN` does **not** require an API redeploy. Do not create a new backend release merely to refresh this secret. Verify the positive Production-origin preflight/read path and a negative foreign-origin preflight after the secret update.
...
The source-level `sloppy-imports` bridge remains useful for Deno validation, but production deployment no longer depends on Supabase's server-side bundler implementing that unstable resolution behavior. Node built-in normalization is resolver-level only; the generated JavaScript is not textually rewritten after bundling, so string/template/SQL payloads that merely contain text such as `require("crypto")` are not mutated. The hosted deployment is the final environment-level proof; the first public web production acceptance passed on 2026-09-24 and is recorded in `docs/runbooks/public-web-production-smoke-2026-09-24.md`.
