...
The command refuses a dirty working tree or a mismatch between `DEPLOY_CODE_SHA` and `git rev-parse HEAD`. It then uses pinned `esbuild@0.28.2`, embeds that verified Git SHA directly into the generated code as the service version, bundles the complete Edge dependency graph into `supabase/functions/api/dist/index.js`, rejects leftover relative ESM imports or an unresolved build-SHA placeholder, and fails before deployment if the generated artifact reaches the 5 MB server-side bundle limit.
