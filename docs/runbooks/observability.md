...
This allows an operator to connect a browser-visible failure with one concrete server request without logging request bodies, authorization headers, JWTs or binary content. Telemetry is fail-open: a broken logging sink is swallowed and cannot change a successful business response or suppress a controlled failure response.
...
Readiness executes a minimal PostgreSQL probe inside a bounded, configurable dependency budget (default 1000 ms). A failed or timed-out dependency check returns HTTP 503 with `SERVICE_NOT_READY`. Dependency exception details are not returned to callers. This timeout is intentionally scoped to readiness only; business commands are never wrapped in a generic HTTP timeout because an underlying write could commit after an acknowledgement timeout.
