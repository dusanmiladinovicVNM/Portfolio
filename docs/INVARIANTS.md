...
161. Effective Tenancy terms use the latest valid term version under the same governing legal chain with `effectiveFrom <= asOf`. Exact money and source billing frequency are preserved; Reporting never invents normalized recurring revenue.
...
167. Property reporting rows may contain only Units/current operations belonging to that Property. Property occupancy buckets and portfolio occupancy buckets must each partition their corresponding Unit count.
