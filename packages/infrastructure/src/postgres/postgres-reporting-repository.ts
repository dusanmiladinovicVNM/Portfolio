...
export class PostgresReportingRepository implements ReportingRepository {
  ...
  async getUnitOverview(
    ...
    const [snapshotRows, costRows] = await Promise.all([
      this.sql<UnitSnapshotRow[]>`
        select *
        from public.reporting_unit_snapshots(${asOf}::date)
        where unit_id = ${unitId}
        limit 1
      `,
      this.sql<CostSummaryRow[]>`
  ...
  async getPortfolioDashboard(asOf: DateOnly): Promise<PortfolioDashboard> {
    const [propertyRows, costRows, operationRows] = await Promise.all([
      this.sql<PropertySummaryRow[]>`
        select *
    ...
    if (!ops) {
      throw new Error('Portfolio reporting operations projection returned no row.');
    }
